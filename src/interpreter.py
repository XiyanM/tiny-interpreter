...
from .ast_nodes import Literal, Grouping, Binary, Expr, Unary, Variable, Assign, Call
    ...
    def _eval(self, expr: Expr) -> Any:
        ...
        if isinstance(expr, Call):
            ...
            for arg in expr.args:
                arguments.append(self._eval(arg))

            if not isinstance(callee, LoxFunction):
                raise RuntimeError_(
                    f"Can only call functions at line {expr.paren.line}."
                )

            if len(arguments) != callee.arity():
                raise RuntimeError_(
                    f"Expected {callee.arity()} arguments but got {len(arguments)} at line {expr.paren.line}."
                )
